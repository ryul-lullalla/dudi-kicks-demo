...
import { floor } from "@/lib/number/format";
...
export const math = (value: Argument) => {
  ...
  // let res = new Big(typeof val === 'bigint' ? (val as BigInt).toString() : val);
  ...
  return {
    ...
    // Format
    format: function (size: number) {
      const k = new Big(10).pow(size);
      res = res.div(k);
      return this;
    },
    // Parse
    parse: function (size: number) {
      const k = new Big(10).pow(size);
      res = res.mul(k);
      return this;
    },
